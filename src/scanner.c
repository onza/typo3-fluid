...
static bool custom_tag_ends_with(const Tag *tag, const char *suffix) {
    if (tag->type != CUSTOM) {
        return false;
    }
    size_t suffix_len = strlen(suffix);
    const String *name = &tag->custom_tag_name;
    return name->size >= suffix_len &&
           memcmp(name->contents + name->size - suffix_len, suffix, suffix_len) == 0;
}
...
static bool scan_raw_text(Scanner *scanner, TSLexer *lexer) {
    ...
    } else if (top->type == STYLE) {
        end_delimiter = "</STYLE";
    } else if (custom_tag_ends_with(top, ":ASSET.CSS") || custom_tag_ends_with(top, ":ASSET.SCRIPT")) {
        // "</" + uppercase custom name, e.g. </F:ASSET.CSS
        if (2 + top->custom_tag_name.size >= sizeof(end_buf)) {
            return false;
        }
        end_buf[0] = '<';
        end_buf[1] = '/';
        memcpy(end_buf + 2, top->custom_tag_name.contents, top->custom_tag_name.size);
        end_buf[2 + top->custom_tag_name.size] = '\0';
        end_delimiter = end_buf;
    } else {
        return false;
    }
...
static bool scan_start_tag_name(Scanner *scanner, TSLexer *lexer) {
    ...
    array_push(&scanner->tags, tag);
    // Fluid <f:asset.css> / <f:asset.script>: opaque body like HTML style/script
    // so CSS/JS `{` is not parsed as a Fluid expression.
    if (custom_tag_ends_with(&tag, ":ASSET.CSS")) {
        lexer->result_symbol = STYLE_START_TAG_NAME;
    } else if (custom_tag_ends_with(&tag, ":ASSET.SCRIPT")) {
